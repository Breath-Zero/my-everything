...
public class FileIndexDaoImpl implements FileIndexDao {
    ...
    public void insert(Thing thing) {
        ...
        PreparedStatement statement = null; // 准备命令
        ...
        try {
            ...
            //FileType.DOC  ->  DOC  (文件只需要根据类型得到枚举的名称)
    ...
    public List<Thing> search(Condition condition) {
        ...
        Connection connection = null; // 准备连接
        PreparedStatement statement = null; // 准备命令
        ResultSet resultSet = null; // 查询结果集样
        ...
        try {
            ...
            //name     ：  like （需要模糊匹配）
            //fileType : =      （文件类型用=）
            //limit    : limit offset  （限制数量）
            //orderbyAsc : order by     （排序）
...
//            System.out.println(sqlBuilder.toString());
            ...
            //4.设置参数 1 2 3 4
            //5.执行命令
            resultSet = statement.executeQuery(); // 更新
            //6. 处理结果
            while (resultSet.next()) {
                //数据库的行记录--->Java中的对象（Thing对象）
                Thing thing = new Thing();
                resultSet.getString("name");
                thing.setName(resultSet.getString("name"));
                thing.setPath(resultSet.getString("path"));
                thing.setDepth(resultSet.getInt("depth")); //深度
